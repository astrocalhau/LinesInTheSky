...
function add_MBH_Hb_WuShen2022!(cc)
    ...
    mask = ((cc.QSOcont_norm .> cc.QSOcont_norm_unc)              .&
            (cc.QSOcont_alpha .> -5) .& (cc.QSOcont_alpha .<  5)  .&
            (.!isnan.(cc.L5100))                                  .&
            (cc.Hb_br_norm .> cc.Hb_br_norm_unc)                  .&
            (.!isnan.(cc.Hb_br_fwhm_unc))
            )
    i = findall(.!ismissing.(mask)  .&  mask)
...
function add_MBH_MgII_WuShen2022!(cc)
    ...
    mask = ((cc.QSOcont_norm .> cc.QSOcont_norm_unc)              .&
            (cc.QSOcont_alpha .> -5) .& (cc.QSOcont_alpha .<  5)  .&
            (.!isnan.(cc.L3000))                                  .&
            (cc.MgII_2798_br_norm .> cc.MgII_2798_br_norm_unc)    .&
            (.!isnan.(cc.MgII_2798_br_fwhm_unc))
            )
    i = findall(.!ismissing.(mask)  .&  mask)
...
function add_MBH_Ha_ShenLiu2012!(cc)
    ...
    mask = ((cc.Ha_br_norm .> cc.Ha_br_norm_unc)    .&
            (.!isnan.(cc.Ha_br_fwhm_unc))
            )
...
function add_Lbol_eddratio!(cc)
    ...
    for i in 1:nrow(cc)
    try
	    cc[i, :Lbol_mean] = mean(skipmissing([cc[i, :Lbol_3000], cc[i, :Lbol_5100]]))
    catch
